...
#define SYS_MY_DENCRYPT 552  // El número de syscall que registraste en syscall_64.tbl
...
int main(int argc, char *argv[]) {
    ...
    long result = syscall(SYS_MY_DENCRYPT, input_file, output_file, key_file, num_threads);
    if (result == -1) {
        perror("Error executing syscall");
        return EXIT_FAILURE;
    }

    printf("File dencrypted successfully.\n");
    return EXIT_SUCCESS;
}
